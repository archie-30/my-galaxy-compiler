...
def handle_run_code(data):
    ...
    if lang == 'python':
        if not main_python_file:
            emit('program_output', {'data': "❌ 錯誤: 找不到 Python 執行檔。\r\n"})
            emit('program_status', {'status': 'error'})
            return
        ...
        if not cpp_sources:
            emit('program_output', {'data': "❌ 錯誤: 找不到 C++ 原始碼。\r\n"})
            emit('program_status', {'status': 'error'})
            return
        exe_file = os.path.join(workspace, "galaxy_runner")
        compiler = shutil.which("clang++") or shutil.which("g++")
        if not compiler:
            emit('program_output', {'data': "❌ 錯誤: 找不到編譯器。\r\n"})
            emit('program_status', {'status': 'error'})
            return
        log(f"正在編譯: {compiler}")
        compile_cmd = [compiler] + cpp_sources + ['-o', exe_file]
        compile_res = subprocess.run(compile_cmd, capture_output=True, text=True, env=my_env, cwd=workspace)
    ...
    try:
        ...
        log(f"啟動進程: {run_cmd}")
        current_process = subprocess.Popen(run_cmd, stdin=slave_fd, stdout=slave_fd, stderr=slave_fd, preexec_fn=os.setsid, close_fds=True, env=my_env, cwd=cwd)
        os.close(slave_fd)
